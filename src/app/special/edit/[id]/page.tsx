...
export default function EditSpecialPage() {
  ...
  useEffect(() => {
    ...
    if (status === "authenticated" && specialId) {
      Promise.all([
        fetch("/api/posts").then(res => res.json()),
        fetch(`/api/specials/${specialId}`).then(res => res.json())
      ]).then(([postsData, specialData]) => {
        setAvailablePosts(Array.isArray(postsData) ? postsData : (postsData.posts || []));
        
        if (specialData) {
          setFormData({
            title: specialData.title || "",
            description: specialData.description || "",
            tags: specialData.tags || "",
            is_main: specialData.is_main || 0,
          });
          setPreviewImageUrl(specialData.bg_image_url || "");
          if (specialData.posts) {
            setSelectedPosts(specialData.posts);
          }
        }
        setLoading(false);
      }).catch(err => {
  ...
  const handleAddPost = (post: any) => {
    ...
    if (selectedPosts.find(p => p.id === post.id)) return;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/specials/${specialId}`, { method: "PUT", body: data });
      if (res.ok) {
        alert("성공적으로 스페셜이 수정되었습니다!");
        router.push("/special");
      } else {
  ...
  if (loading) {
    return <div style={{ background: "#e8e8e8", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}><p>Loading Special Data...</p></div>;
  }
    ...
    <div style={{ fontFamily: "'Georgia', serif", background: "#f5f5f0", minHeight: "100vh", padding: "2rem" }}>
      ...
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2rem", borderBottom: "2px solid rgba(0,0,0,0.05)", paddingBottom: "1rem" }}>
        <div>
          ...
          <h1 style={{ fontSize: 32, fontWeight: 900, fontStyle: "italic", margin: "0.5rem 0 0", color: "#111", letterSpacing: "-0.05em" }}>EDIT SPECIAL.</h1>
        </div>
        <button disabled={saving} onClick={handleSubmit} style={{ background: saving ? "#ccc" : "#27c93f", color: "#fff", border: "none", padding: "0.8rem 2rem", borderRadius: 50, fontSize: 13, fontWeight: 900, fontStyle: "italic", cursor: saving ? "not-allowed" : "pointer", boxShadow: "0 8px 24px rgba(39,201,63,0.3)" }}>
          {saving ? "SAVING..." : "UPDATE SPECIAL"}
        </button>
      </header>
      ...
      <div style={{ display: "flex", gap: "2rem", alignItems: "flex-start" }}>
        ...
        <div style={{ flex: 1, border: `1.5px solid #27c93f`, borderRadius: 12, padding: "1.75rem", background: "#fff" }}>
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
            ...
            <div>
              <label style={labelStyle}>🖼 Top Background Image</label>
              <div onClick={() => document.getElementById("img-upload")?.click()} style={{ background: "#f9f9f9", border: `2px dashed ${selectedFile ? "#27c93f" : "#ddd"}`, borderRadius: 12, padding: "1.5rem", textAlign: "center", cursor: "pointer" }}>
                <p style={{ color: selectedFile ? "#27c93f" : "#888", fontSize: 13, fontStyle: "italic", fontWeight: "bold", margin: 0 }}>
                  {selectedFile ? `✓ ${selectedFile.name} (Click to change)` : "배경 이미지 변경 (선택)"}
                </p>
              </div>
